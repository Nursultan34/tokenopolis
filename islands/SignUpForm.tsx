...
export default function SignUpForm() {
  ...
  return (
    <main class="w-full lg:(flex flex-row) h-screen bg(login-bg cover dark:none)">
      <LoginHeader />
      <form class="login-form" method="POST">
        ...
        <div className="flex flex-row gap-x-5">
          ...
          <input required class="login-input w-7/12" name="email" type="email" placeholder="ЭЛЕКТРОННАЯ ПОЧТА" value={email} onInput={(e) => setEmail(e.target?.value)} />
        </div>
        <IncorrectWarning pred={isEmail} val={email} msg="Неверный E-mail!" />
        <input required class="login-input" name="password" type={showPass ? "text" : "password"} placeholder="ВВЕДИТЕ ПАРОЛЬ" value={pass} onInput={(e) => setPass(e.target?.value)} />
...
function IncorrectWarning({ pred, msg, val }: { pred: (_: string) => boolean, msg: string, val: string }) {
  // Warning is not displayed if the email is correct or not entered yet
  if (pred(val) || val == "") {
    return <></>;
  } else {
