...
function IconMessageUser() {
	...
	return (
		<div className="flex flex-row">
			...
			{active && (
				<div className="absolute z-10 bg-dark-3 right-0 opacity-60 top-[10%]" style={{ width: "100%", height: "90%" }}>
				</div>
			)}
			{userState && (
				<div className="bg-white absolute z-20 right-4 top-24" style={{ width: 320, height: 417 }}>
					<div className="bg-gray-white mt-6 ml-4 flex flex-col justify-start items-center" style={{ width: 288, height: 372 }}>
						<div className="w-full h-1/2 mt-6">
							<div className="w-full flex flex-row justify-center items-center h-1/3" style={{ display: "flex", flex: 1 }}>
								{!langState && (
									<button
										className={`border border-white flex flex-row justify-end items-center bg-white rounded-sm hover:bg-yellow-light hover:border-yellow-light rounded-sm focus:outline-none`}
								...
								{langState && (
									<button
										className={`border flex flex-row justify-end items-center rounded-sm bg-yellow-light border-gray-dashed rounded-sm focus:outline-none`}
										style={{ width: 260, height: 54 }}
										onMouseEnter={() => setMouseStatus(true)}
										onMouseLeave={() => setMouseStatus(false)}
										onClick={() => setLangState((prev) => !prev)}
									>
										<text className="mr-4 flex flex-row justify-center items-center font-light">
											{langState ? <img className="mr-3" src={asset("headerImage/Vector.png")} style={{ width: 7.1, height: 15.84 }} /> : ""}ВЫБРАТЬ ЯЗЫК
										</text>
										<img className="mr-3.5" src={asset("headerImage/translate.png")} style={{ width: 25.08, height: 25.08 }} />
									</button>
								)}
								{langState && (
									<div className="bg-white absolute right-[calc(20.3rem)] flex justify-center items-center" style={{ width: 164, height: 174 }}>
										<div style={{ width: 140, height: 134 }} className="flex flex-col justify-center items-center rounded-sm">
											<div
												className="flex border border-white flex-auto bg-gray-white justify-center items-center hover:border-gray-bg"
												style={{ width: 140, height: 42 }}
											>
												английский
											</div>
											<div
												className="flex border border-white flex-auto bg-gray-white justify-center items-center mt-1 hover:border-gray-bg"
												style={{ width: 140, height: 42 }}
											>
												украинский
											</div>
							...
							<div className="flex w-full justify-center items-center h-1/3" style={{ display: "flex", flex: 1 }}>
								<div
									className="flex flex-row bg-white border border-white justify-end items-center hover:bg-yellow-light hover:border-yellow-light rounded-sm"
									style={{ width: 260, height: 54 }}
								>
							...
							<div className="flex w-full justify-center items-center h-1/3 pl-3.5 pr-3.5" style={{ display: "flex", flex: 1 }}>
								...
								<div
									className={`flex flex-row bg-white ml-2 border border-white justify-end items-center hover:bg-yellow-light hover:border-yellow-light rounded-sm ${
							...
							<div className="flex w-full flex-row justify-center items-center justify-center items-center h-1/3" style={{ display: "flex", flex: 1 }}>
								<div
									className="flex flex-row justify-end items-center border border-white bg-white hover:bg-yellow-light hover:border-yellow-light"
									style={{ width: 260, height: 54 }}
								>
