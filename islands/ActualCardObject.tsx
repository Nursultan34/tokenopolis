...
export default function ActualCardObjectComponent (props : ICardObjectComponent) {
    ...
    return(
        <div className={"w-full bg-gray-light flex flex-row rounded justify-center items-center mt-2 hover:bg-gray-coo hover:shadow-side"} style={{height: props.cardType === 'all' ? 120 : 100}} onClick={()=>console.log('ddddddd')}>
            ...
            <div className={`flex flex-col ${props.cardType === 'all' ? '' : 'ml-2'}`} style={{display: 'flex', flex: 1}}>
                ...
                { props.cardType === 'all' &&<div className="flex flex-row mt-1">
            ...
            {props.cardType === 'all'&&<div className="flex flex-col justify-center items-center" style={{display: 'flex', flex: 1}}>
                <button className="border border-gray-cool bg-white-light rounded-sm text-sm font-light hover:bg-gray-cool" style={{width: '10rem', height: 40}}>подробнее</button>
                <button className="border mt-1 border-yellow-orange bg-yellow-orange rounded-sm font-light hover:bg-yellow-main" style={{width: '10rem', height: 40}}>ИНВЕСТИРОВАТЬ</button>
            </div>}
            {props.cardType === 'my'&&<div className="flex justify-center items-center"  style={{display: 'flex', flex: 1}}>
