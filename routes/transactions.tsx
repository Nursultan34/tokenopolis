...
export const handler: Handlers = {
    GET: mayFail(async (req, ctx) => {
        ...
        const transactions = await stellar.getTransactions(
            await stellar.server.loadAccount(
                keypair.publicKey(),
            ),
            0,
        );
        const _operations = await Promise.all(
            transactions.map((tx) => tx.operations()),
        );
        const infer_is_incoming = (o) => ({
            target:
                o.from === keypair.publicKey() ? o.to : o.from,
        ...
        const operations = R.pipe(
            ...
            R.map(
                R.over(R.lensProp('created_at'), (d) =>
        ...
        <article class="flex flex-col h-screen">
            <div class="lg:hidden flex justify-between font-bold text-sm pb-2">
                ...
                <span class="text-[10px] font-normal">
                    Всего{' '}
                    <span class="text-xs font-bold"> 2345</span>
                </span>
            </div>
            <div class="row gap-x-4 mb-4">
                <div class="flex items-center lg:text-base text-sm">
                    ...
                    <input
                        type="text"
                        value="08/12/2022"
                        class="lg:w-36 w-[92px] lg:ml-1 lg:mr-2 lg:px-6 py-2.5 text-xs lg:text-lg lg:font-medium px-3 mx-3 rounded-sm"
                    />
                    <img
            ...
            <div class="flex items-center lg:text-base lg:pr-10 lg:gap-x-6 border-b pb-1 border-[#D1D1D1] lg:border-none mb-2 text-sm">
                ...
                <span class="text-gray-dark mr-6">
                    ИСХОДЯЩИЕ{' '}
                </span>
