...
export default function Faq() {
	return screenWrapper(
		<article class="w-full h-full row gap-4">
...
function FaqSection() {
	...
	return (
		<section class="pt-[3vh] pl-[2vw]">
			<h2 class="font-bold" style="font-size:calc(0.7vw + 0.7vh + 0.5vmin);">
				Частые вопросы
			</h2>
			<div class="col items-start mt-[1.5vh] mr-4 h-full">
				<div
					class={`w-full h-[75vh] col items-start gap-[1vh] overflow-y-scroll ${
						tw(
							scrollbarStyle,
						)
					}`}
					...
					{questions.map((question) => (
						...
						>
							<div class="row justify-between p-[2vh] pr-8">
								...
								<img src={asset("cross.svg")} class="w-[1.3vw]" />
							</div>
						</div>
					))}
				</div>
...
function ArticlesSection() {
	...
	return (
		<section class="py-[3vh] px-[2.5vw]">
			<h2 class="font-bold mb-[1.5vh]" style="font-size:calc(0.7vw + 0.7vh + 0.5vmin);">
				Обучающие статьи
			</h2>
			<div class={`w-full h-[46vh] col gap-[1vh] overflow-y-scroll ${tw(scrollbarStyle)}`}>
				{articles.map((article) => (
					<a href={article.href}>
						<div class="w-[36vw] h-[10vh] row items-center justify-start border border-gray-cool rounded">
							<img src={asset(article.img)} class="w-[4vw] m-[0.5vw]" />
							<p class="w-[56%] uppercase mr-[7vw]">{article.title}</p>
							<img src={asset("arrow-article.svg")} class="w-[2vw]" />
						</div>
					</a>
				))}
			</div>
